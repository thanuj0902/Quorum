...
function getSocials(): Social[] {
  ...
  return [
    ...
    {
      name: 'X',
      href: `https://x.com/intent/tweet?text=${encodeURIComponent(SHARE_TEXT)}&url=${encodeURIComponent(url)}`,
      icon: (
        <svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor">
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-2 p-2 bg-surface border border-border rounded-xl shadow-2xl shadow-black/30 z-50"
          >
            <div className="flex items-center gap-1.5">
              {socials.map((s) => (
                <a
                  ...
                  className="w-9 h-9 flex items-center justify-center rounded-lg text-text-secondary transition-all duration-200 hover:bg-surface-2 hover:text-text"
              ...
              <button
                onClick={handleCopy}
                className="w-9 h-9 flex items-center justify-center rounded-lg text-text-secondary transition-all duration-200 hover:bg-surface-2"
